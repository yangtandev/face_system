...
- **Amend Safety Protocol:** Before using `git commit --amend`, you MUST run `git reflog -n 5` and `git status` to verify the HEAD commit message and staged changes. Ensure you are amending the *intended* commit and not a distinct logical unit.
- **Explicit Citation Strategy:** Before executing any critical action (modify, commit, document), you MUST explicitly cite the specific rule from `AGENTS.md` that governs that action (e.g., "According to Rule 13.9 Mandatory Syntax Check, I will..."). This forces you to re-read and internalize the protocols in real-time.
